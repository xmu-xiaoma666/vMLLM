...
class MaskCLIP(nn.Module):
    ...
    def decode_features(self, class_features, image_features, text_features=None):
        ...
        cls_logits_per_image = logit_scale * image_features @ class_features.t()
        ...
        cls_logits_per_image = cls_logits_per_image.permute(0, 2, 1)
        B, C, Nt = cls_logits_per_image.shape
        cls_logits_per_image = cls_logits_per_image.reshape(B, C, int(math.sqrt(Nt)), int(math.sqrt(Nt)))

        return logits_per_class, logits_per_image, cls_logits_per_image
    ...
    def forward(self, image, text=None, normalize=True, text_feature_layer=None):
        ...
        logits_per_class, logits_per_image, cls_logits_per_image = self.decode_features(
