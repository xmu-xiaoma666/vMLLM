...
class ProjectBlock(nn.Module):
    def __init__(self, mm_hidden_size,hidden_size,project_size):
        super().__init__()

        self.proj = nn.Sequential(
            nn.Linear(project_size*3, hidden_size),
            nn.GELU(),
        ...
        self.q_proj = nn.Linear(project_size,project_size)
        self.k_proj = nn.Linear(project_size,project_size)
        self.v_proj = nn.Linear(project_size,project_size)

        self.txt_q_proj = nn.Linear(project_size,project_size)
        self.txt_k_proj = nn.Linear(project_size,project_size)
        self.txt_v_proj = nn.Linear(project_size,project_size)
...
def build_vision_projector(config, delay_load=False, **kwargs):
    ...
    mlp_gelu_match = re.match(r'^mlp(\d+)x_gelu$', projector_type)
    if mlp_gelu_match:
        return ProjectBlock(config.mm_hidden_size, config.hidden_size,512)    
